...
public class ejercicio1Insersion {
...
// para hacer una inserción.
    ...
    private static int[] insertaPosicion(int inicio[],int valor,int posicion) {
        ...
        for(int i=0;i<=posicion;i++){
            if(i==posicion){
                paraDevolver[i]=valor;
            }
            else
                paraDevolver[i]=inicio[i];
        }
        for(int i=posicion;i<inicio.length;i++){
            paraDevolver[i+1]=inicio[i];
        }
        // Fin del algoritmo
        return paraDevolver;
    }
    ...
    private static int[] insertaOrdenado(int inicio[],int valor) {
        ...
        resultado=insertaPosicion(inicio,valor,posicion);
        return resultado;
    }
